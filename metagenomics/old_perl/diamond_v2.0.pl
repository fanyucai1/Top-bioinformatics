...
my $diamond="/allwegene1/work/fanyucai/software/diamond/diamond";
my $nr_index="/allwegene2/database/diamond_index/nr";
my $Viruses_index="/allwegene2/database/diamond_index/Viruses";
my $idba_ud="/allwegene1/work/fanyucai/software/IDBA/idba-master/bin";
my $blast2lca="/allwegene1/work/fanyucai/software/MEGAN/megan6/tools/blast2lca";
my $g2t="/allwegene1/work/fanyucai/software/MEGAN/prot-gi2tax-August2016X.bin";
my $a2t="/allwegene1/work/fanyucai/software/MEGAN/prot-acc2taxid-August2016.abin";
my $R="/allwegene1/work/fanyucai//software/R/R-v3.2.4/bin/";
my $lib="export LD_LIBRARY_PATH=/allwegene1/work/fanyucai/software/lib:\$LD_LIBRARY_PATH";
my $metaphlan="/allwegene1/work/fanyucai/software/metaphlan/metaphlan2/";
my $lefse="/allwegene1/work/fanyucai/metagenomics/sub/lefse.pl";
my $xvf_run="/allwegene1/work/fanyucai/software/bin/xvfb-run";
my($outdir,@prefix,@pe1,@pe2,@fa,$num,$type,$index,$db,@class,@subclass);
...
for(my $k=0;$k<=$#prefix;$k++)
{
    if($type eq "nr")
    {
            print D2 "$xvf_run --auto-servernum --server-num=1  $blast2lca -g2t $g2t -i $outdir/$prefix[$k].daa -o $outdir/$prefix[$k].tax.out\n";
    }
    else
    {
            print D2 "$xvf_run --auto-servernum --server-num=1  $blast2lca -a2t $a2t -i $outdir/$prefix[$k].daa -o $outdir/$prefix[$k].tax.out\n";
    }
 }
&qsub("$outdir/diamond2.sh","blade.q");
...
##################################################get the Species Accumulation Curves and rarefaction use vegan

##################################################get the barplot of species
