...
my $megahit="/allwegene3/work/fanyucai/software/megahit/megahit-master/megahit";#v1.0.6
my $fastuniq="/allwegene3/soft/public/source/FastUniq/fastuniq";
my $qsub="/allwegene3/work/fanyucai/software/qsub/qsub_sge.pl";

my (@pe1,@pe2,$outdir,$minlength,@prefix,$presets,$queue);
$minlength||=500;
my $thread||=80;
$queue||="big.q";
GetOptions(
...
-o           output directory
-queue	     set which queue run the tasks.
...
chop($metape1);
chop($metape2);
&qsub("$outdir/fastuniq.sh",$queue);
###################step2:assembly use megahit
if($#pe1<=100)
{
    if($#pe1<5)
    {
    	system "echo '$megahit -1 $metape1 -2 $metape2 --presets meta-sensitive  --min-contig-len $minlength -m 0.3 -t $thread -o $outdir/megahit'>$outdir/megahit.sh";
    }
    elsif($#pe1>=5 && $#pe1<=10)
    {
    	system "echo '$megahit -1 $metape1 -2 $metape2 --presets meta --min-contig-len $minlength  -m 0.3 -t $thread -o $outdir/megahit'>$outdir/megahit.sh";
    }
    elsif($#pe1>10 && $#pe1<15)
    {
    	system "echo '$megahit -1 $metape1 -2 $metape2 --presets meta-large --min-contig-len $minlength  -m 0.3 -t $thread -o $outdir/megahit'>$outdir/megahit.sh";
    }
    else
    {
    	system "echo '$megahit -1 $metape1 -2 $metape2 --presets bulk --min-contig-len $minlength -m 0.3 -t $thread -o $outdir/megahit/'>$outdir/megahit.sh";
    }
}
&qsub("$outdir/megahit.sh","$queue");
