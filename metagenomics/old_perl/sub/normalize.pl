...
use Getopt::Long;
use FindBin qw($Bin);
my (@pe1,@pe2,$outdir,@sampleID,$prefix,@sam,$fa,@depth,$count,$ass_maxproc,$queue);

my $fastq_qc_stat="$Bin/fastq_qc_stat";
#my $env="export LD_LIBRARY_PATH=/allwegene3/work/fanyucai/software/gsl/gsl-v1.13/lib:/allwegene1/work/fanyucai/software/R/R-v3.2.4/lib64/R/lib:\$LD_LIBRARY_PATH";
my $sam2count="$Bin/sam2counts.py";
my $python="/allwegene3/soft/public/python/Python-v2.7.13/bin/python";
my $trinity="/allwegene3/soft/public/trinityrnaseq/trinityrnaseq_r20140717";
...
-ass_maxproc	set the num of progress onece running.
...
for(my $k=0;$k<=$#left;$k++)
{
    ...
    print SAM "$fastq_qc_stat -a $left[$k] -b $right[$k] -f $outdir/$id[$k] -q 100\n";
}
&qsub("$outdir/noramlize.sh",$queue,$ass_maxproc);
